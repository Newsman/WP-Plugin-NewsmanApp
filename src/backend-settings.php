		...
		<section id="tabSettings" class="tab-panel">
			<div class="wrap wrap-settings-admin-page">
				<form method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin.php?page=NewsmanSettings' ) ); ?>">
					...
					<h2><?php echo esc_html__( 'Elementor', 'newsman' ); ?></h2>
					<table class="form-table newsman-table newsman-tbl-fixed">
						<tr>
							<th scope="row">
								<label class="nzm-label" for="newsman_elementor_active"><?php echo esc_html__( 'Use Elementor Integration', 'newsman' ); ?></label>
							</th>
							<td>
								<input name="newsman_elementor_active" type="checkbox"
									id="newsman_elementor_active" <?php echo ( ! empty( $form_values['newsman_elementor_active'] ) && 'on' === $form_values['newsman_elementor_active'] ) ? 'checked' : ''; ?>/>
						...
						<?php
						if ( $this->is_elementor_pro_exists() ) :
							$elementor_export_on = ( ! empty( $form_values['newsman_elementor_export_subscribers'] ) && 'on' === $form_values['newsman_elementor_export_subscribers'] );
							$elementor_forms     = $this->get_elementor_newsletter_forms();
							?>
							<tr>
								<th scope="row">
									<label class="nzm-label" for="newsman_elementor_export_subscribers"><?php echo esc_html__( 'Export Subscribers from Form Submissions', 'newsman' ); ?></label>
								</th>
								<td>
									<input name="newsman_elementor_export_subscribers" type="checkbox"
										id="newsman_elementor_export_subscribers" <?php echo $elementor_export_on ? 'checked' : ''; ?>/>
									<p class="description"><?php echo esc_html__( 'When enabled, the Newsman subscriber.list API v1 endpoint pulls subscribers from Elementor Pro form submissions (wp_e_submissions) instead of from WordPress users / WooCommerce orders. Pick the source form below.', 'newsman' ); ?></p>
								</td>
							</tr>
							<tr style="display: <?php echo $elementor_export_on ? 'table-row' : 'none'; ?>;">
								<th scope="row">
									<label class="nzm-label" for="newsman_elementor_export_form_id"><?php echo esc_html__( 'Source Form', 'newsman' ); ?></label>
								</th>
								<td>
									<select name="newsman_elementor_export_form_id" id="newsman_elementor_export_form_id">
										<option value=""><?php echo esc_html__( '— select a form —', 'newsman' ); ?></option>
										<?php foreach ( $elementor_forms as $widget_id => $label ) : ?>
											<option value="<?php echo esc_attr( (string) $widget_id ); ?>" <?php selected( (string) $form_values['newsman_elementor_export_form_id'], (string) $widget_id ); ?>>
												<?php echo esc_html( (string) $label ); ?>
											</option>
										<?php endforeach; ?>
									</select>
									<?php if ( empty( $elementor_forms ) ) : ?>
										<p class="description"><em><?php echo esc_html__( 'No Elementor forms are marked as newsletter forms yet. Open a form in the editor and turn on both "Send to Newsman" and "Newsletter form" to make it available here.', 'newsman' ); ?></em></p>
									<?php else : ?>
										<p class="description"><?php echo esc_html__( 'Only one form per store is supported. The 8-char ID shown matches what appears in Elementor admin - Submissions - Form column.', 'newsman' ); ?></p>
									<?php endif; ?>
								</td>
							</tr>
						<?php endif; ?>
					...
					<h2><?php echo esc_html__( 'Contact Form 7', 'newsman' ); ?></h2>
					<table class="form-table newsman-table newsman-tbl-fixed">
						<tr>
							<th scope="row">
								<label class="nzm-label" for="newsman_contact_form_7_active"><?php echo esc_html__( 'Use Contact Form 7 Integration', 'newsman' ); ?></label>
							</th>
							<td>
						...
						<?php
						if ( $this->is_flamingo_exists() ) :
							$cf7_export_on = ( ! empty( $form_values['newsman_contact_form_7_export_subscribers'] ) && 'on' === $form_values['newsman_contact_form_7_export_subscribers'] );
							$cf7_forms     = $this->get_cf7_newsletter_forms();
							?>
							<tr>
								<th scope="row">
									<label class="nzm-label" for="newsman_contact_form_7_export_subscribers"><?php echo esc_html__( 'Export Subscribers from Form Submissions', 'newsman' ); ?></label>
								</th>
								<td>
									<input name="newsman_contact_form_7_export_subscribers" type="checkbox"
										id="newsman_contact_form_7_export_subscribers" <?php echo $cf7_export_on ? 'checked' : ''; ?>/>
									<p class="description"><?php echo esc_html__( 'When enabled, the Newsman subscriber.list API v1 endpoint pulls subscribers from Flamingo Inbound Messages of the selected Contact Form 7 form instead of from WordPress users / WooCommerce orders.', 'newsman' ); ?></p>
								</td>
							</tr>
							<tr style="display: <?php echo $cf7_export_on ? 'table-row' : 'none'; ?>;">
								<th scope="row">
									<label class="nzm-label" for="newsman_contact_form_7_export_form_id"><?php echo esc_html__( 'Source Form', 'newsman' ); ?></label>
								</th>
								<td>
									<select name="newsman_contact_form_7_export_form_id" id="newsman_contact_form_7_export_form_id">
										<option value=""><?php echo esc_html__( '— select a form —', 'newsman' ); ?></option>
										<?php foreach ( $cf7_forms as $cf7_post_id => $label ) : ?>
											<option value="<?php echo esc_attr( (string) $cf7_post_id ); ?>" <?php selected( (string) $form_values['newsman_contact_form_7_export_form_id'], (string) $cf7_post_id ); ?>>
												<?php echo esc_html( (string) $label ); ?>
											</option>
										<?php endforeach; ?>
									</select>
									<?php if ( empty( $cf7_forms ) ) : ?>
										<p class="description"><em><?php echo esc_html__( 'No Contact Form 7 forms are marked as newsletter forms yet. Open a CF7 form, go to the Newsman tab, and turn on both "Send to Newsman" and "Newsletter form" to make it available here.', 'newsman' ); ?></em></p>
									<?php else : ?>
										<p class="description"><?php echo esc_html__( 'Only one form per store is supported. Flamingo persists every submission of the selected form to the flamingo_inbound channel matching the form slug.', 'newsman' ); ?></p>
									<?php endif; ?>
								</td>
							</tr>
						<?php endif; ?>
					...
					<h2><?php echo esc_html__( 'WPForms', 'newsman' ); ?></h2>
					<table class="form-table newsman-table newsman-tbl-fixed">
						<tr>
							<th scope="row">
								<label class="nzm-label" for="newsman_wpforms_active"><?php echo esc_html__( 'Use WPForms Integration', 'newsman' ); ?></label>
							</th>
							<td>
								<input name="newsman_wpforms_active" type="checkbox"
									id="newsman_wpforms_active" <?php echo ( ! empty( $form_values['newsman_wpforms_active'] ) && 'on' === $form_values['newsman_wpforms_active'] ) ? 'checked' : ''; ?>/>
