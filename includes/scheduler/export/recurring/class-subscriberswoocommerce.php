...
class SubscribersWoocommerce extends AbstractScheduler {
	...
	public function is_allow() {
		...
		$allow = $allow && $this->remarketing_config->is_export_woocommerce_subscribers();

		return apply_filters( 'newsman_scheduler_export_recurring_woocommerce_subscribers_allow', $allow );
	}
	...
	public function process_recurring( $data = array(), $blog_id = null ) {
		...
		try {
			...
			if ( 0 >= $count ) {
				$this->logger->info( 'No subscribers from orders to export, date=' . $date_created );
				return false;
			}
	...
	public function schedule_short() {
		...
		$date = new \DateTime();
		$date->sub( new \DateInterval( 'P' . $this->remarketing_config->get_export_woocommerce_recurring_short_days() . 'D' ) );
	...
	public function schedule_long() {
		...
		$date = new \DateTime();
		$date->sub( new \DateInterval( 'P' . $this->remarketing_config->get_export_woocommerce_recurring_long_days() . 'D' ) );
	...
	public function get_recurring_short_interval() {
		$interval = apply_filters(
			'newsman_scheduler_export_recurring_woocommerce_subscribers_short_interval',
	...
	public function get_recurring_long_interval() {
		$interval = apply_filters(
			'newsman_scheduler_export_recurring_woocommerce_subscribers_long_interval',
