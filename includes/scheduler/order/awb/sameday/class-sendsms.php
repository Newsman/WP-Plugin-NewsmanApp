...
namespace Newsman\Scheduler\Order\Awb\Sameday;
...
/**
 * Send SMS for order with SamedayCourier AWB.
 *
 * @class \Newsman\Scheduler\Order\Awb\Sameday\SendSms
 */
class SendSms extends AbstractScheduler {

	/**
	 * Background event hook send order SMS with SamedayCourier AWB
	 */
	public const BACKGROUND_HOOK_EVENT = 'newsman_order_sms_sameday_awb';
	...
	/**
	 * Notify SMS via Newsman with order SamedayCourier AWB
	 *
	...
	/**
	 * Send SMS for order with SamedayCourier AWB via Newsman API
	 *
	 * @param int    $order_id     Order ID.
	 * @param string $awb          Order SamedayCourier AWB.
	...
	public function send_sms( $order_id, $awb, $is_scheduled = false ) {
		...
		$message    = $this->sms_config->get_sms_sameday_awb_message();
		...
		try {
			...
			do_action( 'newsman_order_sms_awb_sameday_send_sms_before', $order, $awb, $is_scheduled );
			...
			$context->set_list_id( $list_id )
				->set_text( $message )
				->set_to( $phone );
			$context = apply_filters( 'newsman_order_sms_awb_sameday_send_sms_context', $context, $order, $awb, $is_scheduled );
			...
			do_action( 'newsman_order_sms_awb_sameday_send_sms_after', $order, $awb, $is_scheduled );
