...
/**
 * API Class Service Subscriber Update
 *
 * @class \Newsman\Service\Subscriber\Update
 */
class Update extends AbstractService {
	/**
	 * Update subscriber Newsman API endpoint.
	 *
	 * @see https://kb.newsman.com/api/1.2/subscriber.update
	 */
	public const ENDPOINT = 'subscriber.update';

	/**
	 * Update subscriber.
	 *
	 * @param UpdateContext $context Subscriber update context.
	 * @return array|string
	...
	public function execute( $context ) {
		...
		$this->logger->info(
			sprintf(
				...
				esc_html__( 'Try to update subscriber %s', 'newsman' ),
				empty( $subscriber_id ) ? $context->get_email() : (string) $subscriber_id
			)
		);

		$client  = $this->create_api_client();
		$context = apply_filters( 'newsman_service_subscriber_update_execute_context', $context );
		$result  = $client->post(
			$api_context,
			array(),
			array(
		...
		$this->logger->info(
			sprintf(
				/* translators: 1: Email or subscriber ID */
				esc_html__( 'Updated subscriber %s', 'newsman' ),
				empty( $subscriber_id ) ? $context->get_email() : (string) $subscriber_id
			)
		);
