 ...
 * to the matching retriever. Priority is: Elementor Pro - CF7 (via Flamingo) - WC -
 * WP. Each step checks its precondition (option on, plugins active, source form is
 * still marked as Newsman + newsletter form); if the precondition fails the chain
 * falls through to the next source. If nothing matches an `ApiV1Exception(3002)`
 * is thrown for API v1 callers, and a legacy exception otherwise.
 *
...
class Subscribers extends AbstractRetriever implements RetrieverInterface {
	...
	public function process( $data = array(), $blog_id = null ) {
		...
		// 3. WooCommerce subscribers (existing).
