...
/**
 * Handle inbound subscriber.subscribe API v1 request.
 *
...
class SubscriberSubscribe extends AbstractRetriever implements RetrieverInterface {
	/**
	 * Action hook fired when subscriber.subscribe is called.
	 */
	public const ACTION_HOOK = 'newsman_subscriber_subscribe';
	...
	public function process( $data = array(), $blog_id = null ) {
		...
		$this->logger->info(
			sprintf(
				...
				esc_html__( 'subscriber.subscribe: %1$s, blog %2$d', 'newsman' ),
				$email,
				(int) $blog_id
			)
		);
		...
		/**
		 * Fires when Newsman requests a subscriber.subscribe action.
		 *
