 ...
 *
 * @class \Newsman\Admin\Settings\Oauth
 */
class Oauth extends Settings {
	/**
	 * Page nonce action
	 *
	 * @var string
	...
	public function process_form_settings_step_two() {
		...
		update_option( 'newsman_list', sanitize_text_field( wp_unslash( $list_id ) ), \Newsman\Config::AUTOLOAD_OPTIONS );
	...
	public function get_remarketing_settings( $list_id, $user_id = null, $api_key = null ) {
		try {
			...
			$context->set_user_id( $user_id )
				->set_api_key( $api_key )
				->set_list_id( $list_id );
			$get_sms_list_all = new \Newsman\Service\Configuration\Remarketing\GetSettings();
