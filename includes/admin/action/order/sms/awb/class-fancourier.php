...
/**
 * Admin order view action send SMS with FAN Courier AWB
 *
 * @class \Newsman\Admin\Action\Order\Sms\Fancourier
 */
class Fancourier {
	/**
	 * Config
	...
	public function init_hooks() {
		...
		add_action( 'woocommerce_order_action_newsman_send_sms_awb_fancourier', array( $this, 'execute' ), 10, 1 );
	}

	/**
	 * Add action to send SMS with FAN Courier AWB in Order actions in admin order view page
	 *
	...
	public function add_menu_action( $actions, $order ) {
		...
		$actions['newsman_send_sms_awb_fancourier'] = __( 'Send SMS with FAN Courier AWB', 'newsman' );
		return $actions;
	}
	...
	public function execute( $order ) {
		...
		try {
			$send_sms = new \Newsman\Scheduler\Order\Awb\Fancourier\SendSms();
